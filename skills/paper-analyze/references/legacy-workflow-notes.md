...
# 读取各章节内容
# 读取各章节内容（使用通配符匹配所有 .tex 文件）
cat /tmp/paper_analysis/*.tex > /tmp/paper_analysis/all_content.txt
```
...
1. **读取图谱数据**
   - 文件路径：`$OBSIDIAN_VAULT_PATH/论文笔记/PaperGraph/graph_data.json`
...
```bash
# 搜索已有笔记
find "${VAULT_ROOT}/论文笔记" -name "*${PAPER_ID}*" -type f
```
